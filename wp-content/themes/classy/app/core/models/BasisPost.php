...
class BasisPost extends Basis {
    ...
    /**
     * Returns array of tag titles of current post or empty array if there is no tags
     *
     * @return array
     */
    public function getTags() {
      $tags = $this->getAcfByKey('acf_tags');
      $tag_titles = [];

      if(isset($tags) && count($tags) > 0) {
        foreach($tags as $tag) {
          array_push($tag_titles, get_post($tag)->post_title);
        }
        return $tag_titles;
      } else {
        return [];
    ...
    // Get ACF field
    public function getAcfByKey($key) {
        if(!function_exists('get_field')) {
            return 'Please activate Advanced Custom Fields PRO plugin';
        }

        return get_field($key, $this->ID);
    }

    // Get image by ACF key - ACF MUST return image ID
    public function getAcfImage($key = 'acf_image') {
        $image = $this->getAcfByKey($key);
	    $image_id = null;
    ...
    public function getFlyImage($key, $size = [100, 100], $crop = null)
    {
        if(!function_exists('fly_get_attachment_image_src')) {
            return 'Please activate Fly Dynamic Image Resizer plugin';
        }

        $attachment_id = $this->getAcfByKey($key)['id'];

        return fly_get_attachment_image_src($attachment_id, $size, $crop)['src'];
    }

    public function getAcfDate($format = 'd F Y')
    {
        return General::getFormattedDate($this->getAcfByKey('acf_date'), $format);
    }

    public function getDate($format = 'd.m.Y H:i')
    {
        return General::getFormattedDate($this->post_date, $format);
    }
